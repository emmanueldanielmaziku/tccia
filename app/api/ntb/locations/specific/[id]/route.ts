...
export async function GET(
  ...
  try {
    ...
    const apiUrl = `${API_BASE_URL}/api/ntb/locations/specific/${id}`;
    console.log("Fetching NTB specific locations from:", apiUrl);
    ...
    if (!response.ok) {
      ...
      console.error(
        "NTB specific locations API response not ok:",
        response.status,
        response.statusText
      );
    ...
    console.error("Error fetching NTB specific locations:", error);
    return NextResponse.json(
      {
        success: false,
