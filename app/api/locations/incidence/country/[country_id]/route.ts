...
export async function GET(
  ...
  try {
    ...
    const apiUrl = `${API_BASE_URL}/api/locations/incidence/country/${country_id}`;
    console.log("Fetching location incidences for country from:", apiUrl);
    ...
    if (!response.ok) {
      ...
      console.error(
        "Location incidences API response not ok:",
        response.status,
        response.statusText
      );
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Failed to fetch location incidences",
          details: errorText,
        },
