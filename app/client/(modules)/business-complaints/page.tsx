...
interface LocationIncidence {
  ...
  name: string;
  display_name: string;
  specific_locations: SpecificLocation[];
  specific_locations_count: number;
}
...
export default function BusinessComplaints() {
  const { isRightSidebarOpen } = useRightSidebar();
  const t = useTranslations("businessComplaints");
  ...
  useEffect(() => {
    if (form.region_id) {
      const selectedRegion = regions.find(r => String(r.id) === form.region_id);
      if (selectedRegion) {
        setDistricts(selectedRegion.districts || []);
      } else {
        setDistricts([]);
      }
      // Reset district and location selections when region changes
      setForm((prev) => ({
        ...prev,
        district_id: "",
        location_of_incidence_id: "",
  ...
  useEffect(() => {
    if (form.region_id) {
      fetchLocationIncidences(parseInt(form.region_id));
      // Reset location selections when region changes
      setForm((prev) => ({
        ...prev,
        location_of_incidence_id: "",
        specific_location_id: "",
  ...
  // Extract specific locations when location of incidence changes
  useEffect(() => {
    if (form.location_of_incidence_id) {
      const selectedLocation = locationIncidences.find(
        (loc) => String(loc.id) === form.location_of_incidence_id
      );
      if (selectedLocation && selectedLocation.specific_locations) {
        setSpecificLocations(selectedLocation.specific_locations);
      } else {
        setSpecificLocations([]);
      }
  ...
  }, [form.location_of_incidence_id, locationIncidences]);
  ...
  const fetchRegions = async () => {
    try {
      const response = await fetch("/api/business_complain/regions");
      const data = await response.json();
      if (data.success) {
  ...
  const fetchLocationIncidences = async (regionId: number) => {
    try {
      const response = await fetch(`/api/locations/incidence/region/${regionId}`);
      const data = await response.json();
      if (data.success) {
  ...
  // Note: fetchSpecificLocations is no longer needed as specific locations
  // are now included in the location incidence response

  const fetchNTBList = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/business_complain/list');
      const data = await response.json();
      if (data.success) {
        setNtbList(data.data || []);
      } else {
        console.error('Error fetching business complaints list:', data.error);
        toast.error(data.error || 'Failed to fetch complaints list');
      }
    } catch (error) {
      console.error('Error fetching business complaints list:', error);
  ...
  const fetchNTBDetails = async (ntbId: number) => {
    ...
    try {
      const response = await fetch(`/api/business_complain/${ntbId}`);
      const data = await response.json();
      if (data.success) {
        setSelectedNtb(data.data);
        setMode('detail');
      } else {
        console.error('Error fetching business complaint details:', data.error);
        toast.error(data.error || 'Failed to fetch complaint details');
      }
    } catch (error) {
  ...
  const detectCurrentLocation = async () => {
    ...
    try {
      ...
      // Note: location_type is not set here as it should match the location_of_incidence type
      // (e.g., "port", "border") rather than urban/rural classification
      setForm(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.region_id || form.region_id.trim() === "") {
    ...
    try {
      ...
      const districtId = parseInt(form.district_id);
      const regionId = parseInt(form.region_id);
      ...
      const payload: any = {
        business_complain_type_id: String(ntbTypeId),
        ...
        location_of_incidence_id: String(locationOfIncidenceId),
        specific_location_id: String(specificLocationId),
      ...
      // Add reporter information from user profile
      if (userProfile) {
        payload.reporter_name = userProfile.name || "";
        payload.reporter_phone = userProfile.phone || "";
        payload.reporter_email = userProfile.email || "";
        payload.reporter_country = userProfile.country_of_residence || "";
        payload.reporter_operator_type = userProfile.operator_type || "";
        if (userProfile.operator_type_other) {
          payload.reporter_operator_type_other = userProfile.operator_type_other;
        }
        payload.reporter_gender = userProfile.gender || "";
      }
      ...
      if (form.exact_loss_value && form.exact_loss_value.trim() !== "") {
        const lossValue = parseFloat(form.exact_loss_value);
        if (!isNaN(lossValue)) {
          payload.exact_loss_value = String(lossValue);
        }
      }
      ...
      // Note: location_type is not included as the API doesn't accept 'urban'/'rural' values
      // If needed, it should be set to valid values like 'port', 'border', etc. based on location_of_incidence
      ...
      // Add files by type (using document and image format as per API)
      ...
      const response = await fetch('/api/business_complain/create-with-files', {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.success) {
        toast.success('Business complaint submitted successfully!');
        setMode('list');
        fetchNTBList();
        resetForm();
      } else {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/business_complain/${feedbackTarget.id}/feedback`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                          ...
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                            <div className="space-y-2">
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="text-xs uppercase tracking-wide text-gray-500">Business Complaint</span>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {selectedNtb.loss_calculation_description && (
                          <div>
                            ...
                            <p className="font-medium whitespace-pre-wrap">{selectedNtb.loss_calculation_description}</p>
                          </div>
                        )}
                      </CardContent>
                    </Card>
                  ...
                  <div className="space-y-6">
                    ...
                    {(() => {
                      // Handle feedback from nested structure
                      const feedback = selectedNtb?.feedback || {};
                      const detailRating = parseRatingValue(feedback?.rating);
                      const detailComment = extractComment(feedback?.additional_comment);
                              ...
                              <div className="flex items-center gap-3 text-green-700">
                                ...
                                {feedback?.feedback_user && (
                                  <span className="text-xs text-gray-500">
                                    by {feedback.feedback_user}
                                  </span>
                                )}
                                {feedback?.feedback_date && (
                                  <span className="text-xs text-gray-500">
                                    on {new Date(feedback.feedback_date).toLocaleDateString()}
                                  </span>
                                )}
                              </div>
                            )}
                        ...
                        <CardContent className="space-y-4">
                          {selectedNtb.attachments.documents && selectedNtb.attachments.documents.length > 0 && (
                            <div>
                              ...
                              <div className="space-y-2">
                                {selectedNtb.attachments.documents.map((doc: any) => (
                          ...
                          {selectedNtb.attachments.images && selectedNtb.attachments.images.length > 0 && (
                            <div>
                              <Label className="text-sm font-medium text-gray-500 mb-2 block">Images</Label>
                              <div className="space-y-2">
                                {selectedNtb.attachments.images.map((img: any) => (
                                  <div key={img.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-md">
                                    <FileText className="w-4 h-4 text-gray-500" />
                          ...
                          {selectedNtb.attachments.videos && selectedNtb.attachments.videos.length > 0 && (
                            <div>
                              <Label className="text-sm font-medium text-gray-500 mb-2 block">Videos</Label>
                              <div className="space-y-2">
                                {selectedNtb.attachments.videos.map((vid: any) => (
                                  <div key={vid.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-md">
                                    <FileText className="w-4 h-4 text-gray-500" />
                                    <span className="text-sm text-gray-700">{vid.name}</span>
                                    {vid.size && (
                                      <span className="text-xs text-gray-500 ml-auto">
                                        {(vid.size / 1024).toFixed(1)} KB
                                      </span>
                                    )}
                                  </div>
                                ))}
                    ...
                    {selectedNtb.final_resolution && (
                      <Card className="border-[0.5px] shadow-[0_0_0px_rgba(0,0,0,0.1)] border-blue-200 bg-blue-50/60">
                        ...
                        <CardContent>
                          <p className="text-sm text-gray-700 whitespace-pre-wrap">{selectedNtb.final_resolution}</p>
                        </CardContent>
                      </Card>
                    )}
                  </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="space-y-6">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mb-4">
                            Upload supporting documents and images to strengthen your business complaint
                          </p>
                        </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl border border-orange-200 animate-in fade-in-0 zoom-in-95 duration-200">
            ...
            <div className="p-4">
              <p className="text-sm text-gray-700 mb-4">
                Please allow location access to submit your business complaint.
              </p>
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                {feedbackTarget
                  ...
                  : "Share your experience with this business complaint."}
              </DialogDescription>
            </DialogHeader>
