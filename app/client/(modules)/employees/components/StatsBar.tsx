...
export default function StatsBar({
  ...
  useEffect(() => {
    ...
    const handleCompanyChangeEvent = () => {
      const updatedCompany = localStorage.getItem("selectedCompany");
      if (updatedCompany) {
        setSelectedCompany(JSON.parse(updatedCompany));
      }
    };
    window.addEventListener("COMPANY_CHANGE_EVENT", handleCompanyChangeEvent);
    window.addEventListener("storage", handleCompanyChangeEvent);
    return () => {
      window.removeEventListener("COMPANY_CHANGE_EVENT", handleCompanyChangeEvent);
      window.removeEventListener("storage", handleCompanyChangeEvent);
    };
  }, []);
